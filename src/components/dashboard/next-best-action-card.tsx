...
interface NextBestActionCardProps {
  ...
  onQuickAction: (habitId: string, data: { status: string; valueCompleted?: number }) => Promise<void>;
  onOpenModal: () => void;
}

export function NextBestActionCard({ action, onQuickAction, onOpenModal }: NextBestActionCardProps) {
  const c = useColors();
  ...
  return (
    <div
      ...
      style={{
        background: `radial-gradient(ellipse at 20% 60%, ${habit.color}12, transparent 55%), ${c.bgCard}`,
        borderColor: `${habit.color}30`,
        boxShadow: `0 0 0 1px ${habit.color}15, ${c.shadow}`,
      }}
    >
      {/* Top accent bar */}
      <div
        className="absolute top-0 left-0 right-0 h-[2px]"
        style={{ background: `linear-gradient(90deg, ${habit.color}, ${habit.color}40)` }}
      />
      ...
      <div className="p-5">
        ...
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center gap-2">
            ...
            <span className="text-xs font-semibold uppercase tracking-wider" style={{ color: habit.color }}>
              Next Action
            </span>
          </div>
          {urgencyType === "streak-risk" && (
            ...
            >
              <Flame className="h-3 w-3" /> Streak at risk
            </div>
          )}
        ...
        <div className="flex items-center gap-4 mb-5">
          ...
          <div className="min-w-0">
            <h3 className="font-bold text-base leading-tight" style={{ color: c.text }}>
              {habit.name}
            </h3>
            <p className="text-sm mt-0.5 leading-snug" style={{ color: c.textMuted }}>
              {remainingCopy}
            </p>
          </div>
        </div>
        ...
        {habit.targetValue !== null && (
          <ProgressBar current={habit.todayCheckIn?.valueCompleted ?? 0} target={habit.targetValue} color={habit.color} c={c} />
        )}
        ...
        <div className="flex items-center gap-2 mt-4">
          <button
            ...
            style={{ backgroundColor: habit.color, color: "white", boxShadow: `0 4px 16px ${habit.color}35` }}
          >
            {loading ? <Loader2 className="h-4 w-4 animate-spin" /> : (
              <>{ctaLabel}{actionType !== "open-modal" && <span style={{ opacity: 0.8 }}>✓</span>}</>
            )}
          </button>
          <button
            onClick={onOpenModal}
            className="flex items-center gap-1 px-3 py-3 rounded-xl text-xs font-medium transition-all hover:brightness-110"
...
function ProgressBar({ current, target, color, c }: { current: number; target: number; color: string; c: ReturnType<typeof useColors> }) {
  const pct = Math.min(100, Math.round((current / target) * 100));
  return (
    <div className="space-y-1.5">
      <div className="flex items-center justify-between">
        <span className="text-xs" style={{ color: c.textMuted }}>{current} / {target}</span>
